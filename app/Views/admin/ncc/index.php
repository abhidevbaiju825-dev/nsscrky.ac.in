...
National Cadet Corps (NCC) Directory
<?= $this->endSection() ?>
...
<div class="mb-6">
    ...
    <p class="text-sm text-gray-500 mt-1">Manage National Cadet Corps personnel and cadets.</p>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
    ...
    <div class="lg:col-span-4">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden sticky top-6">
            ...
            <div class="p-5">
                <form action="<?= base_url('AdminPortal/ncc/store') ?>" method="POST" enctype="multipart/form-data" class="space-y-4">
                    <?= csrf_field() ?>
                    <div>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Designation/Rank <span class="text-red-500">*</span></label>
                        <input type="text" name="designation" class="block w-full rounded-md border-gray-300 border p-2 focus:border-nss-gold focus:ring-nss-gold sm:text-sm" placeholder="e.g. ANO / Senior Under Officer" required>
                    </div>
                    
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Type</label>
                        <select name="type" class="block w-full rounded-md border-gray-300 border p-2 focus:border-nss-gold focus:ring-nss-gold sm:text-sm">
                            <option value="incharge">Incharge / Officer</option>
                            <option value="member" selected>Cadet / Member</option>
                        </select>
                    </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Name & Rank</th>
                            <th scope="col" class="px-5 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Type</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php if(!empty($members)): ?>
                            <?php foreach($members as $m): ?>
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-5 py-3 whitespace-nowrap">
                                        <?php if(!empty($m['_profile_pic'])): ?>
                                            <img src="<?= base_url($m['_profile_pic']) ?>" class="h-10 w-10 rounded-full object-cover shadow-sm border border-gray-100">
                                        <?php else: ?>
                                    ...
                                    <td class="px-5 py-3 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?= $m['_type'] === 'incharge' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800' ?>">
                                            <?= esc(ucfirst($m['_type'])) ?>
                                        </span>
                                    </td>
                                    <td class="px-5 py-3 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="<?= base_url('AdminPortal/ncc/delete/'.$m['_ncc_id']) ?>" onclick="return confirm('Remove this NCC member?')" class="inline-flex items-center justify-center w-8 h-8 rounded text-red-500 hover:bg-red-50 hover:text-red-700 transition-colors">
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="4" class="px-5 py-8 text-center text-gray-500 text-sm">No NCC personnel currently documented.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
